...
class Feedback{
	# Class Properties
	protected $settingsFilePath = "/www/wwwtest.willamette.edu/marco/photo/image-processing/settings/settings.txt"; 
	protected $templatePath = "/www/wwwtest.willamette.edu/marco/photo/image-processing/settings/template.txt"; 
	...
	public function printOutlineArray(){
		echo sizeof($this->outlineArray) . "<br>";
		echo $this->imgHeight . ', ' .$this->imgWidth;
		foreach ($this->outlineArray as $key => $value) {
			foreach ($value as $col => $pixel) {
				echo $pixel; 
			}
			echo "<br>"; 
		}
	...
	public function setTemplateSettings(){
		...
		// Fill outline array with zeros
		$this->outlineArray = array(); 
		for ($col=0; $col < $this->imgHeight ; $col++) { 
			$this->outlineArray[$col][] = array(); 
			for ($row=0; $row < $this->imgWidth ; $row++) { 
				$this->outlineArray[$col][$row] = 0;
			}
		} // --end of for loop
	...
	public function validateFacePosition(){
		...
		if ($correctFaceXPos && $correctFaceYPos) {
		 	...
		 	if (array_key_exists('error', $this->feedback_msgs)) {
		 		$tooClose = in_array("You are too close to the camera", $this->feedback_msgs['error']);
		 		$tooFar = in_array("You are too far to the camera", $this->feedback_msgs['error']) || in_array("You are slightly far away from the camera", $this->feedback_msgs['error']);
		 		if ($tooClose || $tooFar) {
		 	...
		 	if ($xDir != "" && $yDir != "") {
		 		...
		 		if ($yDir == "higher") {
		 			$this->feedback_msgs['error'][] = "Your body position is too low";
		 			$this->feedback_msgs['suggestions'][] = "Use the cropping tool to raise your body position and ensure your shoulders fit the frame"; 
		 		} else {
		 			$this->feedback_msgs['error'][] = "Your body position is too high";
		 			$this->feedback_msgs['suggestions'][] = "Use the cropping tool to lower your body position and ensure your shoulders fit the frame";
		 		}
	...
	public function validateBackground(){
		...
		$minBGP = $this->bgPercentage - 25; 
		 ...
		 if (($imageBGPercent >= $minBGP) && ($imageBGPercent > 40)) {
			...
			$this->feedback_msgs['suggestions'][] = "Try uploading a photo with a solid white background color.";
